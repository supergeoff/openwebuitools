"""
Hermes Agent Pipe for Open WebUI

Proxies chat requests to a Hermes Agent API with streaming support.
Translates hermes.tool.progress SSE events into Open WebUI status
indicators for real-time visual feedback during tool execution.

Install: Admin Panel > Functions > + > Paste this code, then enable.
Remove: Admin Panel > Functions > Delete this function.

User valves (API key, URL, etc.) are configured per-user in
Workspace > Functions > this function > User Valves tab.
"""
...
# ── Admin-level settings (model identity, visible to all users) ────────────
...
# ── User-level settings (personal connection details) ──────────────────────
...
class Pipe:
    ...
    async def pipe(
        ...
        """
        Stream a chat completion from Hermes, translating tool progress
        events into Open WebUI status indicators.
        """
        # ── Resolve user valves ────────────────────────────────────────
        ...
        if not api_key:
            yield "[Hermes] No API key configured. Set your hermes_api_key in User Valves."
            return
        ...
        # Strip manifold prefix if present (e.g. "nous.hermes" -> "hermes")
        if "." in model_id:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        yield (
                            f"[Hermes] Error (HTTP {response.status_code}): "
                    ...
                    async for line in response.aiter_lines():
                        ...
                        # ── Hermes tool progress -> status indicator ────
            ...
            yield "[Hermes] Cannot connect. Check hermes_base_url in your User Valves."
            return
        except httpx.TimeoutException:
            yield "[Hermes] Request timed out."
            return
        except Exception as e:
            yield f"[Hermes] Unexpected error: {str(e)}"
            return
