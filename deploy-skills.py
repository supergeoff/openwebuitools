#!/usr/bin/env python3
"""
Sync SKILL.md-style skills from a manifest (skills.yaml) into an OpenWebUI
instance via its REST API.

Pipeline:
    skills.yaml (list of GitHub URLs)
        -> resolve each URL to one or more (skill_dir, files[]) tuples
        -> flatten each skill into a single markdown blob
        -> upsert via /api/v1/skills/{create,id/{id}/update}

URL formats supported (mirroring Fu-Jie/openwebui-skills-manager):
    - https://github.com/owner/repo               (repo root, discover children)
    - https://github.com/owner/repo/tree/<ref>/<path>  (tree, single or discover)
    - https://github.com/owner/repo/blob/<ref>/<path>/SKILL.md
    - https://raw.githubusercontent.com/owner/repo/<ref>/<path>/SKILL.md

Pinning: branch defaults to 'main' when absent. Drift is accepted.

Env:
    OPENWEBUI_URL       e.g. https://openwebui.example.com
    OPENWEBUI_API_KEY   bearer token from Settings > Account
    GITHUB_TOKEN        optional, lifts GitHub API rate limits to 5000/h

Errors are skipped per skill with a log line; other skills keep going.
Exit code 0 if at least one upsert succeeded and none failed; 1 otherwise.
"""
...
log = logging.getLogger("owui-skills-sync")
...
def _gh_session() -> requests.Session:
    s = requests.Session()
    s.headers["User-Agent"] = "owui-skills-sync/1.0"
...
class OpenWebUI:
    def __init__(self, base_url: str, api_key: str) -> None:
        ...
        self.s.headers["User-Agent"] = "owui-skills-sync/1.0"
...
def main() -> int:
    ...
    ap.add_argument("-m", "--manifest", default="skills.yaml", help="path to skills.yaml")
